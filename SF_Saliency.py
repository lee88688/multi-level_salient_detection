...
cache_out_dir = r'G:\Project\paper2\cache\cache_general_kmeans_300\feature_cache'
...
class SFPlus:
    ...
    def __init__(self):
        ...
        self.__global_sigma2 = 20
        self.__global_sigma3 = 60

        self.__region_sigma1 = 100
        self.__region_sigma2 = 21
    ...
    def uniqueness_original_feature(self, img_segments_mean, coordinate_segments_mean):
        ...
        #     -cdist(coordinate_segments_mean, coordinate_segments_mean) ** 2 / (2 * self.__global_sigma2 ** 2))
        wp_ij = np.exp(
            -cdist(coordinate_segments_mean, coordinate_segments_mean) * self.__global_sigma2)
        wp_ij = wp_ij / wp_ij.sum(axis=1)[:, None]

        uniqueness = np.sum(cdist(img_segments_mean, img_segments_mean) * wp_ij, axis=1);
    ...
    def value_params_original_uniqueness(self, x, *args):
        # if np.abs(x) <= 1:
        #     self.__global_sigma2 = x + 1
        # else:
        #     self.__global_sigma2 = x
        self.__global_sigma2 = np.abs(x)
        feature_list = args[0]
    ...
    def optimize_params_global(self, cache_dir, is_uniqueness=True, x0=None):
        ...
        if is_uniqueness:
            res = optimize.fmin_powell(self.value_params_uniqueness, x0, args=(feature_list,), ftol=1e-3,
                                       callback=callback, disp=True)
        else:
...
if __name__ == "__main__":
    ...
    #     segments = np.load(segments_dir + os.sep + f)
    #     io.imshow(sf1.feature_to_image(sf1.extract_region_features2(feature, region_feature, region_labels)[:, 1], segments))
    #     io.show()
